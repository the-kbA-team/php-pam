 ...
 *
 */
static int chpass_pam_talker(int num_msg, const struct pam_message ** msg, struct pam_response ** resp, void *appdata_ptr) {
  unsigned short i = 0;
...
PHP_FUNCTION(pam_auth) {
  ...
  result = pam_start(PAM_G(servicename), username, &conv_info, &pamh);
